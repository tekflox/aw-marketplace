...
| `resource_estimate` | no | object `{cpu, memory, disk}` — `cpu` is `"low"\|"medium"\|"high"` (traffic-light dot in the UI); `memory` and `disk` are **free-form size estimates** of *server* usage (e.g. `"~200 MB"`, `"1 GB"`) or `"-"` for something that never stays resident (e.g. the git app just installs a CLI). Memory = server RAM (matters for heavy apps like a browser). | Same deal as `publisher` — normally sourced from the app's `aw-app.json resource_estimate` field and merged in server-side; set here only to override. |
